 ...
 * Columns: round, path, wallMs, ok, fail
 */
const fs = require("fs");
...
for (const f of files) {
  ...
  const ok = j.path === "cli" ? (j.exitCode === 0 ? 1 : 0) : j.ok ?? 0;
  rows.push({ round: j.round, path: j.path, wallMs: j.wallMs, ok, fail: j.fail ?? 0 });
}
rows.sort((a, b) => a.round - b.round || a.path.localeCompare(b.path));

const csv = [
  "round,path,wallMs,ok,fail",
  ...rows.map((r) => `${r.round},${r.path},${r.wallMs},${r.ok},${r.fail}`),
].join("\n");
